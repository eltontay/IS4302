...
contract ProfileReview {
    ...
    mapping (address => review[]) serviceReviews; // list of reviews received for each profile for each service provided (review id, milestone completed, star rating out of 5)
    mapping (address => review[]) clientReviews; // list of reviews received for each profile for each service revceived (review id, milestone completed, star rating out of 5)
    ...
    function createReview(address target, string memory review_inpt, uint256 service_id, Role role, uint star_rating) public check_role(target, role, service_id) check_starrating(star_rating) {
        ...
        if (role == Role.provider) { // if the review is for a contractor
            serviceReviews[target].push(newReview);
        } else {
            clientReviews[target].push(newReview);
        }
    ...
    // Get Reviews
    function getServiceReviews(address target) public view returns (review[] memory) {
        return serviceReviews[target];  
    }
    
    function getClientReviews(address target) public view returns (review[]  memory) {
        return clientReviews[target];
    }

    function getAllReviews(address target) public view returns (review[]  memory) {
        uint256 serviceSize = serviceReviews[target].length;
        uint256 clientSize = clientReviews[target].length;
        review[] memory allReviews = new review[](serviceSize + clientSize);

        uint i=0;
        for (;i < serviceSize; i++) {
            allReviews[i] = serviceReviews[target][i];
        }

        uint j=0;
        while (j < clientSize) {
            allReviews[i++] = clientReviews[target][j++];
        }
    ...
    function getSpecificServiceReviews(address target, uint256 service_id) public view returns (review memory) {
        review memory cur_review;

        for (uint256 i = 0; i < serviceReviews[target].length; i++) {
            cur_review =  serviceReviews[target][i];
    ...
    function getSpecificClientReviews(address target, uint256 service_id) public view returns (review[] memory) {
        review[] memory specificedReviews;
        review memory cur_review;
        uint256 reviewCnt = 0;

        for (uint256 i = 0; i < clientReviews[target].length; i++) {
            cur_review =  clientReviews[target][i];
   ...
   // Star Rating getters
    function getAvgServiceStarRating(address target) public view returns (uint256) {
        review[] memory reviews = getServiceReviews(target);
        uint256 sum_star = 0;
    ...
    function getAvgClientStarRating(address target) public view returns (uint256) {
        review[] memory reviews = getClientReviews(target);
        uint256 sum_star = 0;
        uint256 tot_review = reviews.length;
